...
class VideoDataset(tutils.data.Dataset):
    ...
    def _make_lists_road(self):
        ...
        video_frames = [6000, 6000, 3000, 6001]
        i = 0
        for videoname in ['2014-06-26-09-31-18_stereo_centre_02', '2014-12-10-18-10-50_stereo_centre_02', '2015-02-03-08-45-10_stereo_centre_04', '2015-02-06-13-57-16_stereo_centre_01']:
            frames_list = {}
            for fr in range(video_frames[i]):
                frames_list[str(fr+1)] = {'annotated':0}
            final_annots['db'][videoname] = {'split_ids':['final_test'], 'agent_tubes': {}, 'action_tubes': {}, 'loc_tubes': {}, 'duplex_tubes': {}, 'triplet_tubes': {}
                                             , 'av_action_tubes': {}, 'frame_labels': {}, 'frames': frames_list, 'numf': video_frames[i], 'label': 0, 'annotations': []}
            i += 1
        database = final_annots['db']
        ...
        ids_counter = 0
        frame_level_list = []
        #print(database[list(database.keys())[0]]['frames'])
        #A = "2014-06-25-16-45-34_stereo_centre_02"
        #print(database[A]['label_types'])
        #exit()
