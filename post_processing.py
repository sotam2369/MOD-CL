...
if __name__ == "__main__":
    ...
    if not os.path.exists(wdimacs_temp_constraints_path):
        os.makedirs(wdimacs_temp_constraints_path)
    new_data = {}
    for video_name in data.keys():
        new_video_data = {}
        for frame in tqdm(data[video_name].keys()):
            new_frame_data = []
            for bbox in data[video_name][frame]:
                if all(i <= args.th for i in bbox['labels']):  
                    corrected_preds = bbox['labels']
                    cost = 0
                    time_elaps = 0
                else:
                    # Write temporary file containing hard and soft constraints
                    write_temp_file(bbox['labels'], args.th, wdimacs_temp_constraints_path+"th_"+str(args.th)+".txt", wdimacs_constraints_path, args.post_proc,maps=maps)
                    # call the solver on the temporary file
                    cost, new_assgn, time_elaps = call_solver(wdimacs_temp_constraints_path+"th_"+str(args.th)+".txt")            
                    # compute the new predictions
                    corrected_preds = compute_corrected_output(bbox['labels'], new_assgn, args.th)
                # Write on file
                confident_labels = [idx for idx, confidence in enumerate(corrected_preds) if confidence >= args.th]
                if confident_labels != []:
                    new_frame_data.append({'bbox':bbox['bbox'], 'labels':confident_labels})
