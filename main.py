...
import YOLO.tester as tester
import os
import yolo_dataset_maker
import YOLO.extender as extender
...
def main():
    ...
    parser.add_argument('--YOLO', action='store_true', help='Flag for using YOLO Model')
    parser.add_argument('--YOLO_BASEMODEL', default='yolov8x', 
                        type=str, help=' base arch for YOLO')
    parser.add_argument('--YOLO_PRETRAINED', default='', type=str, help=' base arch for YOLO')
    
    parser.add_argument('--MODE', default='train', choices=['train', 'gen_dets', 'eval_frames', 'val', 'extender'],
    ...
    # Every experiment of MWIT is used with the YOLO mode on
    if args.YOLO:
        if args.MODE == 'train':
            # Makes YOLO dataset for training, if not done so already
            if not os.path.isdir(f"../road-dataset/yolo_road-r_task{args.TASK}"):
                yolo_dataset_maker.make_yolo_dataset(task = args.TASK)
            
            print("Starting YOLO training...")

            # Make the folder for saving YOLOv8 models, if not done so already
            if not os.path.isdir(f"./YOLO_models"):
                os.makedirs(f"./YOLO_models")
            
            # Arguments for training the models
            if args.TASK == 2:
                trainer = YOLOTrainer(overrides={"device":"0", "project": f"YOLO_task{args.TASK}", "data":f"./YOLO/dataset_task{args.TASK}.yaml", "task":"detect", "model":f"YOLO_models/{args.YOLO_BASEMODEL}.pt",
                                                "optimizer":"SGD", "lr0": 0.001, "epochs": args.MAX_EPOCHS, "patience": 3}, req_loss=args.LOGIC != "None")
            elif args.TASK == 1:
                trainer = YOLOTrainer(overrides={"device":"0", "project": f"YOLO_task{args.TASK}", "data":f"./YOLO/dataset_task{args.TASK}.yaml", "task":"detect", "model":f"YOLO_models/{args.YOLO_BASEMODEL}.pt",
                                             "optimizer":"SGD", "lr0": 0.001, "epochs": args.MAX_EPOCHS, "close_mosaic": 0}, req_loss=args.LOGIC != "None")

            trainer.train()

        elif args.MODE == 'eval_frames':
            print("Starting YOLO testing...")
            ...
            if args.YOLO_PRETRAINED == '0':
                train_args = ''
            else:
                train_args = args.YOLO_PRETRAINED
            model = YOLO(f'./YOLO_task{args.TASK}/train{train_args}/weights/best.pt')

            # botsort algorithm to use for tracking
            if args.TASK == 1:
                tracking = "YOLO/botsort.yaml"
            else:
            ...
            labelled_videos = "2014-07-14-14-49-50_stereo_centre_01,2015-02-03-19-43-11_stereo_centre_04,2015-02-24-12-32-19_stereo_centre_04".split(',')
            unlabelled_videos = "2014-06-25-16-45-34_stereo_centre_02,2014-07-14-14-49-50_stereo_centre_01," \
                ...
                "2015-03-03-11-31-36_stereo_centre_01".split(",")
            ...
            model = YOLO(f'./YOLO_task{args.TASK}/train{train_args}/weights/best.pt')
            extender.startExtenderTraining(model, labelled_videos, unlabelled_videos, args.DATA_ROOT)
